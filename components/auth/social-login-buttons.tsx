...
export function SocialLoginButtons({ isSignup = false }: SocialLoginButtonsProps) {
  ...
  const handleSocialLogin = async (provider: string) => {
    ...
    // Mock social login - simulate API call
    setTimeout(async () => {
      // In a real app, this would redirect to the provider's OAuth flow
      console.log(`${isSignup ? "Signing up" : "Signing in"} with ${provider}`)

      if (provider === "Google") {
        const success = await login("demo@pup.com", "password")
        if (success) {
          router.push("/dashboard")
        }
      } else {
        // For other providers, show the mock alert
        alert(`${provider} login is mocked - would redirect to OAuth flow`)
      }
