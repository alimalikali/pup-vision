import { Gender, Religion, Profession, PurposeDomain, PurposeArchetype, PurposeModality, Interest , Personality, MaritalStatus, LookingFor, Language, Smoke, Alcohol, Drugs, Politics ,Education } from "@/types/enums"
import { z } from "zod"
...
export const profileSchema = z.object({
  ...
  dob: z.string().or(z.date()).transform((val) => {
  ...
  purposeNarrative: z.string().refine((val) => {
    if (!val || val === "") return true // Allow empty/undefined
    return val.length >= 50 && val.length <= 500
  }, "Narrative must be between 50 and 500 characters if provided").optional(),
  ...
  height: z.number().min(100).max(250).optional().or(z.string().transform((val) => {
    const num = Number(val)
    return isNaN(num) ? undefined : num
  }).refine((val) => !val || (val >= 100 && val <= 250), "Height must be between 100 and 250 cm")),
  weight: z.number().min(30).max(300).optional().or(z.string().transform((val) => {
    const num = Number(val)
    return isNaN(num) ? undefined : num
...
export const profileUpdateSchema = profileSchema.partial().refine((data) => {
